...
func (ahq *ArticleHistoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ArticleHistory, error) {
	...
	if query := ahq.withArticle; query != nil {
		if err := ahq.loadArticle(ctx, query, nodes,
			func(n *ArticleHistory) { n.Edges.Article = []*Article{} },
			func(n *ArticleHistory, e *Article) { n.Edges.Article = append(n.Edges.Article, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (ahq *ArticleHistoryQuery) loadArticle(ctx context.Context, query *ArticleQuery, nodes []*ArticleHistory, init func(*ArticleHistory), assign func(*ArticleHistory, *Article)) error {
	...
	nodeids := make(map[int]*ArticleHistory)
	for i := range nodes {
	...
	query.Where(predicate.Article(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(articlehistory.ArticleColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.article_history_article
		if fk == nil {
			return fmt.Errorf(`foreign-key "article_history_article" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "article_history_article" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (ahq *ArticleHistoryQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(articlehistory.Table, articlehistory.Columns, sqlgraph.NewFieldSpec(articlehistory.FieldID, field.TypeInt))
	_spec.From = ahq.sql
